...
resp_status = requests.post("http://localhost:9200/_template/twitter_hashtags", 
                            data=json.dumps(hashtags))
print resp_status.json()
resp_status.raise_for_status()
...
resp = requests.post("http://localhost:9200/.kibana/index-pattern/logstash-*", 
...
resp = requests.put("http://localhost:9200/.kibana/config/{}".format(resp.json()["hits"]["hits"][0]["_id"]), 
                    data=json.dumps(config))
resp.raise_for_status()

# Load Kibana analytics (Filters, dashboards, etc.)
with open("export.json") as export_file:
    exports = json.load(export_file)

for export in exports:
    print "Loading {} ({})".format(export["_id"], export["_type"])
    resp = requests.post("http://localhost:9200/.kibana/{}/{}".format(export["_type"], export["_id"]), 
                         data=json.dumps(export["_source"]))
    resp.raise_for_status()
